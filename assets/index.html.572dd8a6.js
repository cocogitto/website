...
</code></pre></div><p>The above config would generate a <code>cog commit hotfix</code> and <code>cog commit release</code> subcommands following the same structure as the default ones.</p><p><strong>Overriding existing commit types:</strong></p><p>Existing commit type can be overridden just like custom ones:</p><div class="language-toml ext-toml"><pre class="language-toml"><code><span class="hljs-section">[commit_types]</span>
<span class="hljs-attr">feat</span> = { changelog_title = <span class="hljs-string">&quot;\u2795 Additional features&quot;</span> }
<span class="hljs-attr">fix</span> = { changelog_title = <span class="hljs-string">&quot;\u{1FAB2} Releases&quot;</span> }
</code></pre></div><p><strong>Omit commits from changelog:</strong></p><p>If you want to make changelog more concise you can skip some commit types with the <code>omit_from_changelog</code> option.</p><div class="language-toml ext-toml"><pre class="language-toml"><code><span class="hljs-section">[commit_types]</span>
<span class="hljs-attr">chore</span> = { changelog_title = <span class="hljs-string">&quot;&quot;</span>, omit_from_changelog = <span class="hljs-literal">true</span> }
<span class="hljs-attr">ci</span> = { changelog_title = <span class="hljs-string">&quot;&quot;</span>, omit_from_changelog = <span class="hljs-literal">true</span> }
<span class="hljs-attr">perf</span> = { changelog_title = <span class="hljs-string">&quot;&quot;</span>, omit_from_changelog = <span class="hljs-literal">true</span> }
</code></pre></div><h3 id="deal-with-merge-commits" tabindex="-1"><a class="header-anchor" href="#deal-with-merge-commits" aria-hidden="true">#</a> Deal with merge commits</h3><p>By default, git will write the following message to merge commit : <code>Merge my &#39;branch&#39;</code>. These merge commits do not respect the Conventional Commits specification, and we strongly advise avoiding them by setting the following in your <code>.gitconfig</code> :</p><div class="language-toml ext-toml"><pre class="language-toml"><code><span class="hljs-section">[merge]</span>
...
</code></pre></div><div class="custom-container tip"><p class="custom-container-title">TIP</p><p>You can check your history, starting from the latest tag using <code>--from-latest-tag</code> or <code>-l</code> flag.<br> This is useful when your git repo started to use conventional commits from a certain point in history and you don&#39;t care about editing old commits.</p></div><h2 id="managing-git-hooks" tabindex="-1"><a class="header-anchor" href="#managing-git-hooks" aria-hidden="true">#</a> Managing git-hooks</h2>`,32),O=e("Cocogitto provide a way to share "),z={href:"https://git-scm.com/book/en/v2/Customizing-Git-Git-Hooks",target:"_blank",rel:"noopener noreferrer"},R=e("git hooks"),G=e("."),M=a(`<p>First you need to set up some hooks in <code>cog.toml</code>:</p><div class="language-toml ext-toml"><pre class="language-toml"><code><span class="hljs-comment"># Embedded git-hooks script</span>
<span class="hljs-section">[git_hooks.commit-msg]</span>
<span class="hljs-attr">script</span> = <span class="hljs-string">&quot;&quot;&quot;#!/bin/sh
set -e
cog verify --file $1
cog check
...
<span class="hljs-comment"># Or file path</span>
<span class="hljs-section">[git_hooks.pre-push]</span>
<span class="hljs-attr">path</span> = <span class="hljs-string">&quot;hooks/pre-push.sh&quot;</span>
</code></pre></div><div class="custom-container warning"><p class="custom-container-title">WARNING</p><p>Not that unlike <code>git commit</code>, <code>cog commit</code> will not pick a default shell when running hooks. Make sure to provide a shebang in your hook definition.</p></div><p>Now that our hook are defined in cocogitto&#39;s config they can be installed with <code>cog install-hook</code>.</p><p><strong>Install all git-hooks:</strong></p><div class="language-bash ext-sh"><pre class="language-bash"><code>\u276F <span class="hljs-built_in">cog</span> install-hook --all
</code></pre></div><p><strong>Install a single hook:</strong></p><div class="language-bash ext-sh"><pre class="language-bash"><code>\u276F <span class="hljs-built_in">cog</span> install-hook commit-msg
</code></pre></div><h2 id="sandbox" tabindex="-1"><a class="header-anchor" href="#sandbox" aria-hidden="true">#</a> Sandbox</h2><p><code>cog verify</code> checks an arbitrary input string against the conventional commit specification. It will not create any commit.</p><p><strong>Example:</strong></p><div class="language-bash ext-sh"><pre class="language-bash"><code>\u276F <span class="hljs-built_in">cog</span> verify <span class="hljs-string">&quot;Your Mother Was A Hamster, And Your Father Smelt Of Elderberries&quot;</span>
Error: Missing commit type separator \`:\`
...
</code></pre></div></div><h3 id="custom-templates" tabindex="-1"><a class="header-anchor" href="#custom-templates" aria-hidden="true">#</a> Custom templates</h3>`,7),$=e("If you are not happy with the default you can create your own changelog template. Internally cocogitto uses "),J={href:"https://tera.netlify.app/",target:"_blank",rel:"noopener noreferrer"},U=e("tera"),K=e(" template engine to render changelogs."),Q=a(`<p>Also see <a href="../template">template reference</a>.</p><p><strong>Example:</strong></p><div class="language-tera ext-tera"><pre class="language-tera"><code>{% <span class="hljs-meta">for </span>type, typed_commits in commits | sort(attribute=<span class="hljs-string">&quot;type&quot;</span>)| group_by(attribute=<span class="hljs-string">&quot;type&quot;</span>)%}                            
...
</code></pre></div>`,14),rs={class:"custom-container tip"},ps=s("p",{class:"custom-container-title"},"TIP",-1),hs=e("If any of the pre-bump command fails cocogitto will abort the bump. Any changes made to the repository during the pre-bump phase will be stashed under "),ds=e("."),us=a(`<p><strong>Example:</strong></p><p>cog.toml :</p><div class="language-toml ext-toml"><pre class="language-toml"><code><span class="hljs-attr">pre_bump_hooks</span> = [
  <span class="hljs-string">&quot;cargo build --release&quot;</span>,
...
pre_package_bump_hooks = [
    <span class="hljs-string">&quot;echo &#39;upgrading {{package}}&#39; to {{version}}&quot;</span>,
...
</code></pre></div><h2 id="tag-prefix" tabindex="-1"><a class="header-anchor" href="#tag-prefix" aria-hidden="true">#</a> Tag prefix</h2>`,26),_s=e("It is common to use a tag prefix when creating version in your repository. This is described in the "),ys={href:"https://semver.org/#is-v123-a-semantic-version",target:"_blank",rel:"noopener noreferrer"},ks=e("SemVer specification FAQ"),ws=e(". This convention provide a handy way to distinguish between release versions and tags that does not represent releases."),qs=a(`<p>To tell <code>cog</code> to pick only version starting with a prefix set this in your <code>cog.toml</code> file :</p><div class="language-toml ext-toml"><pre class="language-toml"><code><span class="hljs-attr">tag_prefix</span> = <span class="hljs-string">&quot;v&quot;</span>
</code></pre></div><h2 id="make-cocogitto-skip-ci-cd" tabindex="-1"><a class="header-anchor" href="#make-cocogitto-skip-ci-cd" aria-hidden="true">#</a> Make Cocogitto skip CI CD</h2><p>Cocogitto will create a commit when performing a bump, which can trigger your CI/CD if you have one. Some CI/CD tools support a &quot;skip_ci&quot; string that you can add to a commit which will skip the pipeline execution. To do so with <code>cog</code>, you can use the <code>skip_ci</code> configuration in your <code>cog.toml</code> file or the <code>cog bump --skip-ci &lt;string&gt;</code> option to add a &quot;skip_ci&quot; pattern your CI/CD tool supports.</p><p><strong>Example:</strong></p><div class="language-toml ext-toml"><pre class="language-toml"><code><span class="hljs-attr">skip_ci</span> = <span class="hljs-string">&quot;[skip ci]&quot;</span>
</code></pre></div><p>or using the <code>cog bump</code> command :</p><div class="language-bash ext-sh"><pre class="language-bash"><code>\u276F <span class="hljs-built_in">cog</span> bump --skip-ci <span class="hljs-string">&quot;[skip ci]&quot;</span>
</code></pre></div><p><strong>Result:</strong></p><div class="language-bash ext-sh"><pre class="language-bash"><code>\u276F <span class="hljs-built_in">git</span> log
...
</code></pre></div><p>Note that if both <code>skip_ci</code> configuration and <code>--skip-ci</code> option are used, <code>cog</code> will take the <code>--skip-ci</code> option.</p><h2 id="skip-untracked-or-uncommited-changes" tabindex="-1"><a class="header-anchor" href="#skip-untracked-or-uncommited-changes" aria-hidden="true">#</a> Skip untracked or uncommited changes</h2><p>By default, Cocogitto will abort bump if there are uncommited or untracked changes. You can change this behavior using the <code>skip_untracked</code> configuration in the <code>cog.toml</code> file or the <code>--skip-untracked</code> option of the <code>bump</code> command. If so, the warning will be printed to <code>stderr</code> and the bump will continue.</p><h2 id="get-the-current-version" tabindex="-1"><a class="header-anchor" href="#get-the-current-version" aria-hidden="true">#</a> Get the current version</h2><p>It&#39;s sometime needed to display the current version for scripting purpose. You can print the latest SemVer tag on your repo with the <code>get-version</code> subcommand:</p><div class="language-bash ext-sh"><pre class="language-bash"><code>\u276F <span class="hljs-built_in">cog</span> get-version
Current version:
5.3.1
</code></pre></div><p>To silence the additional info and get only the version use the <code>-v</code> flag:</p><div class="language-bash ext-sh"><pre class="language-bash"><code>\u276F <span class="hljs-built_in">cog</span> -v get-version
5.3.1
</code></pre></div><p>If working on a monorepo you can also specify the target package:</p><div class="language-bash ext-sh"><pre class="language-bash"><code>\u276F <span class="hljs-built_in">cog</span> -v get-version --package gill-db
0.1.0
</code></pre></div><p>Finally, if you need the command to print a version no matter the state of your repository, you can provide a fallback:</p><div class="language-bash ext-sh"><pre class="language-bash"><code>\u276F <span class="hljs-built_in">cog</span> get-version --fallback 0.1.0
0.1.0
</code></pre></div>`,22);function xs(t,As){const o=l("ExternalLinkIcon");return c(),i(p,null,[u,s("p",null,[g,m,s("a",b,[f,n(o)]),v,s("a",j,[_,n(o)]),y]),k,s("p",null,[w,s("a",q,[x,n(o)]),A,C,E]),s("p",null,[T,D,I,P,s("a",H,[N,n(o)]),F]),S,s("p",null,[O,s("a",z,[R,n(o)]),G]),M,s("p",null,[s("a",B,[Y,n(o)])]),W,s("img",{src:t.$withBase("github-release-changelog.png"),alt:"Github release changelog screenshot"},null,8,L),V,s("p",null,[$,s("a",J,[U,n(o)]),K]),Q,s("div",X,[Z,ss,s("ul",null,[es,s("li",null,[as,os,s("a",ns,[ts,n(o)]),ls])]),cs]),is,s("div",rs,[ps,s("p",null,[hs,s("code",null,"cog_bump_"+r(t.version),1),ds]),us]),gs,s("p",null,[ms,s("a",bs,[fs,n(o)]),vs]),js,s("p",null,[_s,s("a",ys,[ks,n(o)]),ws]),qs],64)}var Ts=h(d,[["render",xs]]);export{Ts as default};
